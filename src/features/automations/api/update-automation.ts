...
function buildUpdateAutomationFormData(
...
export async function updateAutomation(
  ...
  try {
    const formData = buildUpdateAutomationFormData(payload);

    const { data } = await apiClient.patch<AutomationApiResponse>(
      `/automations/${id}`,
      formData,
    );
    ...
    throw new Error(
      getApiErrorMessage(error, "Não foi possível atualizar a automação."),
      { cause: error },
    );
  }
}
